 ...
 * 权限约束：成员新增、改角色、移除等管理操作需要 member:manage；分配 OWNER 仍要求操作者是 OWNER；
...
public class MemberService {
    ...
    /**
     * 添加空间成员
     * 权限：需要 member:manage；不能重复添加同一用户
     *
     * @param spaceId 空间ID
    ...
    public MemberVO add(Long spaceId, MemberAddDTO dto) {
        ...
        // DTO转数据库实体，插入成员记录
        SpaceRoleEntity role = spaceRoleService.requireRole(spaceId, dto.roleId());
        requireOwnerAssignmentPermission(spaceId, role);
        MemberEntity member = dto.toEntity(spaceId);
        memberMapper.insert(member);
        return member.toVO(SpaceRoleSummaryVO.from(role));
    }
    ...
    public List<MemberVO> list(Long spaceId) {
        ...
        Map<Long, SpaceRoleEntity> roleMap = spaceRoleMapper.selectBatchIds(
    ...
    /**
     * 修改空间成员角色
     * 权限：需要 member:manage；业务保护：空间不能没有OWNER，唯一OWNER不允许被降级
     *
     * @param spaceId 空间ID
    ...
    public MemberVO changeRole(Long spaceId, Long userId, MemberRoleUpdateDTO dto) {
        ...
        // 获取当前角色信息
        SpaceRoleEntity currentRole = spaceRoleService.requireRole(spaceId, member.getRoleId());
        // 获取新的角色信息
        SpaceRoleEntity newRole = spaceRoleService.requireRole(spaceId, dto.roleId());
        requireOwnerAssignmentPermission(spaceId, newRole);
        // 业务防护：当前用户是OWNER，要降级，且空间仅有这一个OWNER → 禁止降级
        if (OWNER.equals(currentRole.getRoleKey())
                && !OWNER.equals(newRole.getRoleKey())
        ...
        return member.toVO(SpaceRoleSummaryVO.from(newRole));
    }

    /**
     * 移除空间成员
     * 权限：需要 member:manage；业务保护：不允许移除空间最后一名OWNER
     *
     * @param spaceId 空间ID
    ...
    public void remove(Long spaceId, Long userId) {
        ...
        MemberEntity member = requireMember(spaceId, userId);
        // 禁止移除空间仅剩的所有者
        SpaceRoleEntity role = spaceRoleService.requireRole(spaceId, member.getRoleId());
    ...
    private long countOwners(Long spaceId) {
        Long ownerRoleId = spaceRoleService.requireRoleByKey(spaceId, OWNER).getId();
        return memberMapper.selectCount(new LambdaQueryWrapper<MemberEntity>()
                .eq(MemberEntity::getSpaceId, spaceId)
                .eq(MemberEntity::getRoleId, ownerRoleId));
    }

    private void requireOwnerAssignmentPermission(Long spaceId, SpaceRoleEntity role) {
        if (OWNER.equals(role.getRoleKey())) {
