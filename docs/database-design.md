...
## 表清单速览（`V1__init.sql` 基线 25 张，后续迁移增加权限、文档资产、任务草稿、系统能力与组织等表；全新环境执行 V1–V22 后为 41 张业务表）
...
| `V4`–`V12` | 文档资产、文档节点类型、目录树、MCP 连接测试与 Query 参数认证、任务创建契约、关注区域、执行明细、文档 Agent 暂存 |
| `V13__change_request_review_workflow.sql` | 扩展变更审批工作流与评论表，补齐历史文档当前版本快照，并为审批合并增加版本幂等键 |
| `V14`–`V17` | 版本来源元数据、历史数据修复、用量看板与导出权限 |
| `V18__platform_user_department_management.sql` | 新增平台部门表，并为用户补充部门、职位与最后登录时间字段 |
| `V19`–`V22` | 系统能力库（系统 Skill 作用域、空间 Skill 安装、Agent 模板与版本、MCP 模板与版本）及模板版本状态列 |

全新环境按 V1→V22 顺序执行，共 22 个迁移版本；已在验证环境（空库）实测无 checksum、顺序或重复对象错误。

所有环境重建并执行该基线后，`V1__init.sql` 不再修改；V2 及更高版本在基线之后依次增量执行，后续变更继续新增更高版本迁移。
