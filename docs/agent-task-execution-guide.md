...
flowchart TD
    ...
    E["发送到 RabbitMQ 任务队列"]
    F["任务消费者取出任务<br/>去重、取空间投递锁、状态改为已分发"]
...
v0.1 的约束是"一个任务只由一个 Agent 执行"，**不是**"一个空间同一时刻只能跑一个任务"。实测确认：同一空间下两个任务可以同时处于运行中状态。

具体边界如下：

| 环节 | 并发行为 |
| --- | --- |
| MQ 消费与 A2A 投递 | 单实例内消费线程串行（`prefetch: 1`），并对同一空间加 Redis 锁；锁只覆盖"取任务 → 投递 A2A → 回填 A2A 标识"这一小段 |
| Agent 实际执行 | 锁在投递返回后立即释放，模型推理与工具调用发生在 agent-service，**同一空间的任务会真正并发执行** |
| 执行结果回调 | 通过 `POST /api/task/internal/a2a/events` 同步，不经过空间锁 |
| 文档写入 | 不做任务级串行；依靠文档 `baseVersion` 乐观锁防止覆盖 |

空间锁的作用是让**投递阶段**在空间维度串行（例如多实例部署时不重复投递），不是执行期互斥。

由此产生的可预期结果：同一文档被两个并发任务同时修改时，先合并者成功；后合并者基于过期基线会被拒绝并返回冲突，需要重新发起变更。系统不会静默覆盖正文，但会占用一次任务执行。

后续只有在引入工作流或多 Agent 编排并出现新的资源竞争需求时，才重新评估任务级、文档级或资源级并发控制。v0.2.0 的核心工程闭环不以多 Agent 编排为前提，也不会把当前空间投递锁扩展为执行期互斥。

v0.2.0 的多次受控执行继续保持“一项 Task 对应一次 AgentExecution”，通过新的 Task 和显式血缘表达重跑、Replay 或实验变体。详细决策见 [ADR-0001](adr/0001-run-compatible-execution-model.md)。
