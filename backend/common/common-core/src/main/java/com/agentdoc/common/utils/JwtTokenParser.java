 ...
 * <ul>
 * <li>sub：用户ID（人类用户）</li>
 * <li>username：用户名（{@link JwtConstant#CLAIM_USERNAME}）</li>
 * <li>nickname：用户昵称，缺失时回退使用 username（{@link JwtConstant#CLAIM_NICKNAME}）</li>
 * <li>scope：权限作用域，多个作用域使用英文逗号分隔（{@link JwtConstant#CLAIM_SCOPE}）</li>
 * <li>agentId：Agent调用标识，Agent场景下存在，普通用户不存在该claim（{@link JwtConstant#CLAIM_AGENT_ID}）</li>
 * </ul>
 * </p>
 */
public class JwtTokenParser {
    ...
    public LoginUser toLoginUser(Jwt jwt) {
        ...
        String username = jwt.getClaimAsString(JwtConstant.CLAIM_USERNAME);
        String nickname = jwt.getClaimAsString(JwtConstant.CLAIM_NICKNAME);
        // 昵称为空时降级使用用户名
        if (nickname == null || nickname.isBlank()) {
            nickname = username;
        }
        Long agentId = null;
        if (jwt.hasClaim(JwtConstant.CLAIM_AGENT_ID)) {
            Object value = jwt.getClaim(JwtConstant.CLAIM_AGENT_ID);
            agentId = value == null ? null : Long.valueOf(value.toString());
        }
        return new LoginUser(userId, username, nickname, agentId,
                splitScopes(jwt.getClaimAsString(JwtConstant.CLAIM_SCOPE)));
    }
