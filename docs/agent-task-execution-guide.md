...
任务创建使用用户 Access Token。接口入口要求用户登录，业务层通过空间权限服务继续校验成员权限、Agent 归属、文档范围和预算；任务启动后生成唯一可读任务编号，并签发独立的 Task Capability JWT。用户 JWT 的平台角色不会替代任务能力令牌，Agent 也不会继承用户的空间权限。

创建页可以先保存当前用户私有的任务草稿。正式任务支持 `FULL`（可阅读全文）和 `RANGES`（只能读取多个指定区域）两种读取模式；两种模式都可以配置多个关注区域及区域处理要求。区域在创建时固化，`RANGES` 模式下 Workbench MCP 的每次片段读取都必须完整落在某一个授权区域内。能力令牌也按文档类型收敛：草稿文档只签发直接写草稿能力，正式文档只签发创建 ChangeRequest 的能力。
...
flowchart TD
    ...
    C["生成任务记录和可读编号<br/>固化文档类型、读取范围<br/>状态：待运行"]
